    ...
    public async Task Messages([FromBody] AnthropicMessageRequest? request)
    {
        ...
        finally
        {
            if (!string.IsNullOrEmpty(_logContext.Log.Model))
                _activeRequestTracker.EndRequest(_logContext.Log.Model, backend?.Provider?.Id ?? 0, backend?.UnderlyingModelName ?? string.Empty, _logContext.Log.Success, _logContext.Log.Success ? string.Empty : ActiveRequestTracker.GetErrorSummary(_logContext.Log.Answer));
        }
    }
}
